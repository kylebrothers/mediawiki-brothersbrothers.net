...
# pcntl for Scribunto
RUN docker-php-ext-install -j$(nproc) mbstring xml xsl intl mysqli ldap pcntl opcache calendar \
...
RUN EXTS=`curl https://extdist.wmflabs.org/dist/extensions/ | awk 'BEGIN { FS = "\""  } ; {print $2}'` \
    && for i in Arrays HeaderTabs HTMLets ImageMap PageSchemas ParserFunctions Scribunto VisualEditor XSL; do \
